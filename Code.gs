...
function getSessionInfo(){const email=String(Session.getActiveUser().getEmail()||'').toLowerCase();if(email===APP_CONFIG.OWNER_EMAIL.toLowerCase())return{email,role:'OWNER',name:'OWNER'};const pet=findSoalPetugasByEmail_(email);if(pet)return{email,role:APP_CONFIG.ROLE.PETUGAS_SOAL,name:pet.name,soalPermission:{literasi:pet.literasi,numerasi:pet.numerasi}};return findUserByEmail_(email)||{email,role:'UNKNOWN',name:email||'Pengguna'};}
