...
function getSiswaModuleData(kode){
  ...
  return rows.map((r,i)=>Object.assign(rowToModule_(cfg,r),{_rowNumber:i+2})).filter(r=>String(r.email).toLowerCase()===String(s.email).toLowerCase());
}
...
function saveSiswaModule(kode,data){
  ...
  const d=data||{},id='SM-'+Date.now()+'-'+Math.floor(Math.random()*10000);
...
function deleteSiswaModule(kode,id){
  ...
  gatewayDeleteClass_(k.spreadsheetId,cfg.sheet,idx+2);
...
/** Dipanggil OWNER saat membuat/menyiapkan kelas. Siswa tidak pernah membuka SpreadsheetApp secara langsung. */
function ensureAllSiswaModuleSheets_(spreadsheetId){
  const ss=SpreadsheetApp.openById(spreadsheetId);
  Object.keys(SISWA_MODULES).forEach(k=>{
    const cfg=SISWA_MODULES[k];
    let sh=ss.getSheetByName(cfg.sheet);
    if(!sh)sh=ss.insertSheet(cfg.sheet);
    const headers=['id','timestamp','email','nisn','nama','id_kelas'].concat(cfg.fields.map(f=>f[0]));
