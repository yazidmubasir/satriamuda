/** SATRIA MUDA V1.4.2 — canonical module resolution; ALL/SEMUA KELAS + legacy UI safe. */
...
function learningStatusActive_(v){
  const s=String(v==null?'ACTIVE':v).trim().toUpperCase();
  return !s || ['ACTIVE','AKTIF','TRUE','YA','YES','1'].includes(s);
}
function moduleTargetAllowed_(m,idKelas){
  return !!m && learningStatusActive_(m.status) && learningTargetMatch_(m.targetKelas,idKelas);
}
function getLearningModulesRaw_(){
  ...
  return rows.filter(x=>x&&x[0]).map(x=>({
    idModul:String(x[0]).trim(),jenis:String(x[1]||'').trim().toUpperCase(),namaModul:String(x[2]||x[0]).trim(),deskripsi:String(x[3]||''),targetKelas:normalizeLearningTargetValue_(x[4]),status:String(x[5]||'ACTIVE').trim().toUpperCase(),pembuatEmail:String(x[6]||''),pembuatNama:String(x[7]||'')
  }));
}
function findLearningModuleForStudent_(idModul,jenis,idKelas){
  let all=[];
  try{all=getLearningModulesRaw_();}catch(e){all=getMasterModul_(true)||[];}
  ...
  if(requested==='LITERASI'||requested==='NUMERASI'){
    m=all.find(x=>String(x.jenis||'').trim().toUpperCase()===requested && moduleTargetAllowed_(x,idKelas));
    if(m) return m;
  }
  if(type){
    m=all.find(x=>String(x.jenis||'').trim().toUpperCase()===type && moduleTargetAllowed_(x,idKelas));
    if(m) return m;
  }
  return null;
}
function getActiveLearningModulesForStudentFixed(){
  ...
  let all=[];try{all=getLearningModulesRaw_();}catch(e){all=getMasterModul_(false)||[];}
  return all.filter(m=>moduleTargetAllowed_(m,k.idKelas));
}
function startLearningChallengeFixed(idModul,level,count,jenis){
  ...
  let qs=rows.filter(q=>String(q[19]||'').trim().toUpperCase()==='ACTIVE'&&String(q[1]||'').trim()===String(module.idModul).trim()&&Number(q[3])===level&&learningTargetMatch_(q[4],k.idKelas));
...
function getLearningDashboardFixed(){
  const s=getSessionInfo();if(s.role!=='SISWA')return{ok:true,role:s.role};
  const k=findKelasForSession_(s);if(!k)throw new Error('Kelas siswa belum terdaftar.');
  const modules=getActiveLearningModulesForStudentFixed(),bank=readMasterLearningSheet_(APP_CONFIG.SHEET.BANK_SOAL),rows=bank.data&&bank.data.rows?bank.data.rows:[];
  const active=rows.filter(q=>learningStatusActive_(q[19])&&learningTargetMatch_(q[4],k.idKelas));
  const trx=ensureLearningProgressSheet_(k.spreadsheetId),tr=trx.data&&trx.data.rows?trx.data.rows:[],own=tr.filter(x=>String(x[2]).toLowerCase()===String(s.email).toLowerCase()),correct=own.filter(x=>truth_(x[11])).length,xp=own.reduce((a,x)=>a+Number(x[13]||0),0),byType={LITERASI:{correct:0,total:0},NUMERASI:{correct:0,total:0}};
